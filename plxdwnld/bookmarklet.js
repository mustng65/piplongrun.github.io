     ...
     window.plxDwnld = (function() {
         ...
         const getDownloadUrl = function(xml) {
             ...
             if (partKeyNode.singleNodeValue) {
                 ...
                 const url = downloadUrl.replace('{baseuri}', baseUri).replace('{partkey}', partKeyNode.singleNodeValue.textContent).replace('{token}', accessToken);
                 downloadAs(url,'test.mkv')
             } else {
                 alert("You are currently not viewing a media item.");
             }
         };
